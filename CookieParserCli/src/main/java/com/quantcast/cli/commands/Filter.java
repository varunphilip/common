...
public class Filter implements Runnable, IValidate {
	...
	public void validate() {
		...
		if (CookieParserCliMain.results instanceof List<?> == false)
			throw new ParameterException(spec.commandLine(),
					String.format("Invalid inbound Object %n Expected :: List<?> %n Actual "
							+ CookieParserCliMain.results.getClass().getTypeName()));
	...
	public void run() {
		try {
			if (exclusive.cookieId != null)
				service.filterCookies((List<CookieVo>) CookieParserCliMain.results, exclusive.cookieId);
			else
				service.filterCookies((List<CookieVo>) CookieParserCliMain.results, exclusive.date);
		} catch (Exception e) {
