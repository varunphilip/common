...
class CliUtilsTest {
	...
	void testFindActiveCookies() throws Exception {
		...
		CookieParserCliMain.results = reader.read(new File("./test-files/cookies.txt"));
	...
	void testFindActiveCookiesDate() throws Exception {
		...
		CookieParserCliMain.results = reader.read(new File("./test-files/cookies.txt"), dateFormat.parse("2018-12-09"));
		List<String> actuals = new CliUtils().findActiveCookies();	
		assertEquals(expected, actuals);
	}
	...
	void testFilterCookies() throws Exception {
		...
		IFileReader reader = new CsvReader();
		CookieParserCliMain.results = reader.read(new File("./test-files/cookies.txt"));
		List<CookieVo> actuals = new CliUtils().filterCookies((List<CookieVo>)CookieParserCliMain.results, dateFormat.parse("2018-12-09"));
